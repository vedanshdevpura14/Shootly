...
import { X, Calendar, MapPin, FileText, CheckCircle, Clock, Mail, Loader2, Send } from 'lucide-react';
...
export default function EnquireModal({ professional, isOpen, onClose }: EnquireModalProps) {
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl relative animate-fade-in-up my-8">
        ...
        {submitted ? (
          <div className="p-6 text-center space-y-4 flex flex-col items-center justify-center min-h-[280px]">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Inquiry Sent!</h3>
              <p className="text-zinc-400 text-xs mt-1.5">We will get back to you with a quote.</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <div className="flex gap-3 items-center">
              ...
              <div>
                <span className="text-[9px] text-amber-500 font-bold uppercase tracking-widest block">Enquire Talent</span>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <textarea rows={2} required placeholder="Describe requirements..." value={details} onChange={(e) => setDetails(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-white placeholder-zinc-650 focus:outline-none focus:border-amber-600/50 resize-none" />
              </div>
            </div>

            <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-xl text-[11px] text-zinc-400">
              <div className="flex justify-between">
                <span>Starting Rate (Ref)</span>
                <span className="text-white font-medium">
                  ₹{(professional.price ?? professional.startingPrice ?? 0).toLocaleString('en-IN')}
                </span>
              </div>
            </div>

            <button type="submit" disabled={loading} className="w-full bg-white hover:bg-zinc-200 text-zinc-900 py-2.5 rounded-lg text-xs font-semibold transition-colors active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
